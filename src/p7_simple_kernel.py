...
class DataSimple:
    ...
    def get_application(self):
        ...
        # counts_nan_in_cat(df, "Application")
    ...
    def get_bureau_and_balance(self):
        ...
        # counts_nan_in_cat(bureau, "bureau")
        bb = cudf.read_csv(
            os.path.join(DATA_BASE, "bureau_balance.csv"), nrows=self.n_rows
        )
        # counts_nan_in_cat(bb, "bureau_balance")
    ...
    def get_previous_applications(self, nan_as_category=True):
        ...
        # counts_nan_in_cat(prev, "previous_application")
    ...
    def get_pos_cash(self, nan_as_category=True):
        ...
        # counts_nan_in_cat(pos, "POS_CASH_balance")
    ...
    def get_installments_payments(self, nan_as_category=True):
        ...
        # counts_nan_in_cat(ins, "installments_payments")
    ...
    def get_credit_card_balance(self, nan_as_category=True):
        ...
        # counts_nan_in_cat(cc, "credit_card_balance")
    ...
    """def get_train(self, df, verbose=True):
        if self._train_idx is not None:
            ...
            print(
                "L'échantillonage n'est pas créé, utiliser la fonction .set_train_test_idx()"
            )
            return

    def get_test(self, df, verbose=True):
...
"""
    ...
    """def get_test_with_target(self, df):
        test = df[df["TARGET"].isna() == True]
        test = test.drop("TARGET", axis=1)
        application_target = cudf.read_csv(
            os.path.join(DATA_BASE, "application_train.csv"),
            nrows=self.n_rows,
            na_values=["XNA", "Unknown"],
            keep_default_na=True,
        )[["SK_ID_CURR", "TARGET"]]
        test_with_target = test.merge(application_target, how="left", on="SK_ID_CURR")
        return test_with_target"""
    ...
    """def save_data(self, df, verbose=True):
        # On enregistre le train et le test séparément
        train = self.get_train(df)
        filepath_train = os.path.join(self.output_dir, self.train_name)
        train.to_csv(filepath_train, index=False)

        if verbose:
            print(f"Train set enregistré dans {filepath_train}. Forme : {train.shape}")
            print("Informations sur le jeu de Train :")
            train.info()

        test = self.get_test(df, verbose=False)
        filepath_test = os.path.join(self.output_dir, self.test_name)
        test.to_csv(filepath_test, index=False)
        if verbose:
            print(f"\nTest set enregistré dans {filepath_test}. Forme : {test.shape}")
        return train, test"""
    ...
    # Pour nous replacer dans les conditions identiques à la compétition,
    # Nous effaçons la target dans application_train pour la portion qui sera réservée au dataset de Test
    def clear_target_in_testset(self, df_application, verbose=True):
        print(f"Partage Application Train / Test")

        stratify = None
        if self.test_is_stratified:
            stratifiy = df_application["TARGET"]

        X_train, X_test, y_train, y_test = sklearn.model_selection.train_test_split(
            # On souhaite récupérer uniquement les indices
            df_application[["SK_ID_CURR", "TARGET"]],
        ...
        # On efface la Target pour le jeu de test
        df_application.loc[X_test.index, ["TARGET"]] = self.na_value
        if verbose:
            print(
                f"\tStratification : {self.test_is_stratified}, Fraction de test : {self.frac_test:.0%}"
            )
            print(f"\tNombre de lignes dans le Train : {len(X_train)}")
            print(f"\tNombre de lignes dans le Test : {len(X_test)}")
...
def counts_nan_in_cat(df, name=""):
    df_cat = df.select_dtypes(include="object")
    if df_cat is not None:
        n_nan = df_cat.isna().sum().sum()
    else:
        n_nan = 0
    print(f"{name} : Nombre de nan dans les catégorielles = {n_nan}")
