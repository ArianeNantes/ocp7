...
from util import highlight_selected_columns
...
def load_explainer():
    ...
    explainer_path = os.path.join(DATA_DIR, EXPLAINER_NAME)
...
def load_global_importances():
    ...
    path_importances = os.path.join(DATA_DIR, GLOBAL_IMPORTANCES_NAME)
...
def main():
    ...
    url_predict = "https://credit-score2-ejgde3bqeaa7axgn.canadacentral-01.azurewebsites.net/predict"
            ...
            if selected_risk:
                if selected_risk in categorical_features:
                    ...
                    if client_serie[selected_risk] is not None:
                        ...
                        text_to_display += (
                            f"Cela joue en sa défaveur pour obtenir le prêt"
                        )
                    else:
    ...
    def plot_bivariate_boxplot():
        ...
        with col_hist:
            ...
            plotly_style_title(
                f"Histogramme {selected_numeric} par {selected_category}"
            )
