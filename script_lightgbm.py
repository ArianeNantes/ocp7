...
def main(debug=True):
    ...
    # Ajout
    """for feature in lgbm_categorical_feat:
        print("categorical feature :", feature)
        print("min", np.min(df[feature]), "max", np.max(df[feature]))"""
...
def kfold_lightgbm_sklearn(data, categorical_feature=None):
    ...
    for n_fold, (train_idx, valid_idx) in enumerate(
        ...
        if not categorical_feature:
            ...
            for feature in categorical_feature:
                ...
                print("max in train", np.max(train_x[feature]))"""
            clf.fit(
                train_x,
                train_y,
                eval_set=[(train_x, train_y), (valid_x, valid_y)],
    ...
    if GENERATE_SUBMISSION_FILES:
        ...
        test[["SK_ID_CURR", "TARGET"]].to_csv(
            "submission{}.csv".format(SUBMISSION_SUFIX), index=False
        )
        mean_importance.to_csv(
            "feature_importance{}.csv".format(SUBMISSION_SUFIX), index=False
        )
